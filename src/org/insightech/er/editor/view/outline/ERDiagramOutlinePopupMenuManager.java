...
public class ERDiagramOutlinePopupMenuManager extends MenuManager {
    ...
    public ERDiagramOutlinePopupMenuManager(final ERDiagram diagram, final ActionRegistry actionRegistry, final ActionRegistry outlineActionRegistry, final EditPartViewer editPartViewer) {
        try {
            ...
            final MenuManager viewModeMenu = new MenuManager(ResourceString.getResourceString("label.outline.view.mode"));
            ...
            final MenuManager orderByMenu = new MenuManager(ResourceString.getResourceString("label.order.by"));
            ...
            addMenuListener(new IMenuListener() {
                ...
                public void menuAboutToShow(final IMenuManager manager) {
                    try {
                        final List selectedEditParts = editPartViewer.getSelectedEditParts();
                        if (selectedEditParts.isEmpty()) {
                            for (final IContributionItem menuItem : getItems()) {
                                if (menuItem.getId() != null && !menuItem.getId().equals(ChangeOutlineViewToPhysicalAction.ID) && !menuItem.getId().equals(ChangeOutlineViewToLogicalAction.ID) && !menuItem.getId().equals(ChangeOutlineViewToBothAction.ID) && !menuItem.getId().equals(ChangeOutlineViewOrderByPhysicalNameAction.ID) && !menuItem.getId().equals(ChangeOutlineViewOrderByLogicalNameAction.ID)) {
                            ...
                            for (final Class clazz : ACTION_MAP.keySet()) {
                                ...
                                if (!clazz.isInstance(editPart)) {
                                    ...
                                    if (CreateSequenceAction.ID.equals(actionId) && !DBManagerFactory.getDBManager(diagram).isSupported(DBManager.SUPPORT_SEQUENCE)) {
                                        enabled(actionId, false);
    ...
    private void enabled(final String id, final boolean enabled) {
